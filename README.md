...
- **6 tabs**:
  ...
  - **Timeline** — the schedule-aware bar chart, in its own tab. Drag the **middle** of a bar to move the task in time (its duration stays the same); drag its **left or right edge** to resize it (shortening/extending from that side, recalculating duration in business days). Dragging any bar — even one whose dates were only ever calculated from the chain — commits explicit start/end dates for it. A plain click (no drag) opens that task's edit window. **Moving a task drags its continuous descendants along with it**: if a child (or grandchild, and so on) starts right when its parent finishes, with no gap, it shifts by the same amount to preserve that continuity; a child with deliberate slack, or one whose dates are only ever chain-calculated, is left alone (the latter recomputes on its own automatically). A row of category chips above the chart lets you **highlight** one or more categories (including their subcategories) — matching tasks stay at full opacity while everything else dims, without hiding anything. Gets more breathing room when the sidebar is collapsed.
