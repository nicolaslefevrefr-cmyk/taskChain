...
- **Lock a project** to prevent accidentally editing it while you think you're on a different one. Hover a project's sidebar row and click 🔒/🔓 among its other actions (rename/duplicate/export/delete) to toggle it — kept there deliberately, alongside those, so it can't be triggered by a stray click while just switching between projects. A locked project shows a small 🔒 next to its name in the sidebar at all times, and — when it's the active one — an unmissable "🔒 Locked" badge next to the project name in the header. A locked project can still be viewed and switched to, but every mutating action is blocked: creating/editing/deleting tasks, changing statuses, dragging cards or tree boxes, category edits, renaming or deleting the project itself.
- **Categories**: a dedicated "Category" tab lets you build a classic collapsible tree of categories (create, rename, delete, reorder with ↑/↓ among siblings, and drag one onto another to nest it — or onto the empty background to move it back to the top level). Assign a task to one or more categories from its edit window, and when picking a task's **parent tasks**, the list is grouped into that same collapsible category tree instead of one long flat list — much easier to navigate once a project has a hundred-plus tasks. Categories are no longer forced alphabetical — whatever order you set with ↑/↓ is what's used everywhere they're listed, including the Timeline (beta) tab's grouping. This is purely an organizational aid: categories don't feed the scheduling engine or any cascade logic.
- **Tasks**: a large, two-column edit window — a big editable title front and center (no separate label, just shows "Title" as a hint when empty) with a description box underneath, a status/priority/link/start-date/end-date/duration panel on the left (the three date fields stay linked — see below), history (date + note), and an automatic ID (`T-1`, `T-2`, …).
- **Priority** (Low/Medium/High) — set from a toggle in the task's edit window, shown as a sortable column in the List tab too.
- **Categories and parent tasks** are shown as compact inline chips in the task's edit window, each with its own "Edit" button that opens a dedicated picker: categories open the full collapsible category tree with checkboxes, parent tasks open the category-grouped picker. Parent-task chips are clickable — click one to jump straight into that task's own edit window.
...
- **6 tabs**:
  ...
  - **Gantt** — the Kanban board: drag a card to another status column (you'll be asked whether to also apply the change to its child tasks). Each column can be individually collapsed to a narrow strip (handy once you have several statuses) — a collapsed column still accepts a dropped card. The board scrolls horizontally instead of overflowing if there isn't room for every column at once.
  - **Timeline** — the schedule-aware bar chart, in its own tab, listing **every task in the project** — including ones with no dates yet, grouped at the bottom below a divider. Drag the **middle** of a bar to move the task in time (its duration stays the same); drag its **left or right edge** to resize it (shortening/extending from that side, recalculating duration in business days). Dragging any bar — even one whose dates were only ever calculated from the chain — commits explicit start/end dates for it. For a task with no dates yet, its row still shows up with an empty track — drag across it to set a start/end directly, instead of typing dates by hand in the edit window. A plain click (no drag), on a bar or an empty row, opens that task's edit window. **Moving a task drags its continuous descendants along with it**: if a child (or grandchild, and so on) starts right when its parent finishes, with no gap, it shifts by the same amount to preserve that continuity; a child with deliberate slack, or one whose dates are only ever chain-calculated, is left alone (the latter recomputes on its own automatically). A "🎯 Highlight categories" button opens a checkbox tree (handy once you have many categories) to pick which ones stay at full opacity in the chart — everything else dims without being hidden, and the button's own label shows how many are currently highlighted. A "Freeze order" checkbox, top right, stops rows from continuously re-sorting by date as you drag — handy since re-sorting mid-drag is what made a bar jump around vertically; turn it off again to go back to a live chronological order. Gets more breathing room when the sidebar is collapsed.
  - **Timeline (beta)** — the same chart, grouped under a collapsible category tree instead of one flat list (click a header to collapse it); a task tagged with several categories appears once under each. Category names stay visible while scrolling horizontally, same as task names do. Uncategorized tasks are listed last. Shares its zoom level and category highlight with the regular Timeline tab; "Freeze order" here is tracked independently, per category group. Everything else — drag to move/resize/create, click to open — works the same. This is an experimental layout kept as its own tab so it's easy to ignore or remove later if it doesn't end up being useful; it doesn't touch the underlying task data any differently than the regular Timeline tab.
...
- **Start date, end date, and duration are all linked**, in the task's edit window and by dragging a bar in the Timeline tab: editing any one recalculates whichever of the other two it can (in business days) — editing duration keeps the start fixed and moves the end, editing either date recalculates duration if the other date is already set, or the missing date if duration is already set.
- **Parallel vs. sequential planning** (per project, in the Settings tab): by default, tasks that share the same parent are scheduled in **parallel** — they can all start the moment the parent finishes. Switch a project to **sequential** and they're chained instead, one after another in a stable order, as if a single person worked through them one at a time — the first starts when the parent finishes, the second when the first finishes, and so on. This uses a proper dependency-respecting ordering (not just a simple date shift), so it stays correct even when a task's own schedule is independently affected by a completely different, later-resolving dependency elsewhere in the project. In sequential mode, dragging a task earlier than its previous sibling allows is clamped at that boundary rather than silently drifting — a small ⏳ icon appears on the bar (with a tooltip) naming the specific task it's waiting on, and a toast explains it the moment a drag hits that limit. A task's own edit window has an "Ignore sequential planning for this task" checkbox to free it from this constraint entirely, regardless of the project's Planning mode.
- **Chain-aware scheduling ("retro-planning") built into the Timeline tab**: a task doesn't need its own start/end date to appear on the timeline. Dates propagate through the whole dependency graph in **both directions**, repeatedly, until nothing changes — a task with children but no dates of its own gets an end date calculated backward from them, and a task with no dates or children of its own can still get scheduled *forward* the moment one of its parents becomes known. Solving one task's date this way can be exactly what unlocks a sibling or a task further along the chain, so the app re-passes over the whole project until it reaches a fixed point instead of computing everything in one shot. Calculated dates show as dashed bars on the timeline and in gray in the List/edit-modal. If a task's own end date is later than what its descendants require, it's flagged as a conflict (red outline / warning icon).
- **Weekends are excluded** from every date calculation (durations, calculated dates, bar lengths), and a bar that spans a weekend is visually **split in two** (with a thin connector) rather than drawn as one continuous rectangle, so the pause is obvious at a glance.
...
```
`statuses` lives at the workspace level (shared by every project); `locked`, `sequentialPlanning`, and `categories` live on each project; `description`, `priority`, `startDate`/`endDate`/`duration`, and `categories` live on each task — the three date fields are kept in sync by the app (editing any one recalculates whichever of the other two it can). A **single-project** export (from a project's "⬇" icon, in the sidebar or in its Settings tab) is just one entry from that `projects` array on its own — this is also the older format used before multi-project support (which used a single `deadline` field instead of `startDate`/`endDate`), so old exports still import fine and get migrated automatically.

In the example above `T-2` has an explicit end date; `T-1` has none, so the app automatically calculates one for it (business days only) from `T-2`'s start date and duration.
...
- The scheduling engine uses this same relationship in reverse for calculated dates: for a child task to meet its start date, its parent must finish by `child start date − 1 business day`. This is computed recursively up the whole dependency chain, and is what powers the calculated (dashed) bars in the Timeline tab.
