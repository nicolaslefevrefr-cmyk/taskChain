...
- **4 tabs**:
  - **List** — filterable/searchable table with inline editing. Click any of the ID/Title/Status/Deadline/Duration column headers to sort by it (click again to flip ascending/descending — the arrow shows the current sort). The "Depends on" column shows both the ID and the title of each parent (e.g. `T-1: Design`).
