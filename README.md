...
- **Tasks**: a large, two-column edit window — a big editable title front and center (no separate label, just shows "Title" as a hint when empty) with a description box underneath, a status/priority/link/deadline/duration panel on the left, history (date + note), and an automatic ID (`T-1`, `T-2`, …).
