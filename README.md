...
- **6 tabs**:
  ...
  - **Timeline (beta)** — the same chart, grouped under a collapsible category tree instead of one flat list (click a header to collapse it); a task tagged with several categories appears once under each. Uncategorized tasks are listed last. Shares its zoom level and category highlight with the regular Timeline tab; "Freeze order" here is tracked independently, per category group. Everything else — drag to move/resize/create, click to open — works the same. This is an experimental layout kept as its own tab so it's easy to ignore or remove later if it doesn't end up being useful; it doesn't touch the underlying task data any differently than the regular Timeline tab.
